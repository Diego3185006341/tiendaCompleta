...
public class UsuarioDAO {
	...
	public UsuarioDTO iniciarSesion(String nombre, String clave) {
		...
		try {
			...
			ps = cnn.prepareStatement("SELECT U.usuario, U.password FROM usuarios U WHERE U.usuario= ? AND U.password = ?");
			ps.setString(1, nombre);
			ps.setString(2, clave);
			rs = ps.executeQuery();
			
			while (rs.next()) {
				usu= new UsuarioDTO(0);
